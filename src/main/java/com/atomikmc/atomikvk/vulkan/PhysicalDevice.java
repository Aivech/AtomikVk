...
final class PhysicalDevice {
    ...
    private static int scoreDevice(VkPhysicalDevice device, long khrSurface, CharSequence[] requiredExtensionNames) {
        try(MemoryStack stack = stackPush()) {
            ...
            var requiredExtensions = new HashSet<CharSequence>(Arrays.asList(requiredExtensionNames));
            ...
            if(pFormatCount.get(0) == 0 || pPresentModeCount.get(0) == 0) return Integer.MIN_VALUE; // missing required formats or modes
